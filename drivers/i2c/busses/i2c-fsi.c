			 ...
			 bool stop)
{
	...
	return fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_CMD, cmd);
}
...
static int fsi_i2c_write_fifo(struct fsi_i2c_port *port, struct i2c_msg *msg,
			      u8 fifo_count)
{
	int bytes_to_write = port->ctrl->fifo_size - fifo_count;
	...
	while (bytes_to_write) {
		...
		rc = fsi_device_write(port->ctrl->fsi, I2C_FSI_FIFO,
			     ...
			     u8 fifo_count)
{
	...
	while (bytes_to_read) {
		...
		if (xfr_remaining) {
			rc = fsi_device_read(port->ctrl->fsi, I2C_FSI_FIFO,
...
static int fsi_i2c_get_scl(struct i2c_adapter *adap)
{
	...
	fsi_i2c_read_reg(port->ctrl->fsi, I2C_FSI_STAT, &stat);
...
static void fsi_i2c_set_scl(struct i2c_adapter *adap, int val)
{
	...
	if (val)
		fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_SET_SCL, 0);
	else
		fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_RESET_SCL, 0);
}
...
static void fsi_i2c_set_sda(struct i2c_adapter *adap, int val)
{
	...
	if (val)
		fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_SET_SDA, 0);
	else
		fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_RESET_SDA, 0);
}
...
static void fsi_i2c_prepare_recovery(struct i2c_adapter *adap)
{
	...
	rc = fsi_i2c_read_reg(port->ctrl->fsi, I2C_FSI_MODE, &mode);
	if (rc)
		return;

	mode |= I2C_MODE_DIAG;
	fsi_i2c_write_reg(port->ctrl->fsi, I2C_FSI_MODE, mode);
}
				 ...
				 struct i2c_msg *msg, u32 status)
{
	...
	if (status & I2C_STAT_ERR) {
		port->ctrl->skip_stop = status & I2C_STAT_SKIP_STOP;
